...
int main() {
    ...
    thrust::device_vector<uint64_t> in_bounds(N);
    
    thrust::sequence(in_bounds.begin(), in_bounds.end(), 0);
    ...
    thrust::transform(in_bounds.begin(), in_bounds.end(), in_bounds.begin(), transformation);

    uint64_t found = thrust::reduce(in_bounds.begin(), in_bounds.end());
